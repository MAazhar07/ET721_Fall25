...
class Fruit(db.Model):
    ...
    expiration_date = db.Column(db.Date, nullable=True)  # Optional for safety
    ...
    def validate_expiration_date(self, key, exp_date):
        if exp_date is not None:
            assert isinstance(exp_date, date), 'Expiration must be a valid date object'
        return exp_date

    @staticmethod
    def search(name=None, variety=None, season=None, min_quantity=None,
               max_quantity=None, expired_only=False):
        query = Fruit.query
        if name:
        ...
        if expired_only:
            today = date.today()
            query = query.filter(Fruit.expiration_date < today)
        return query.all()
