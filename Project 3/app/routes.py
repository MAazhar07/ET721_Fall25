...
def update_fruit(fruit_id):
    ...
    # Update fruit reference in the DB with new values
...
# NEW ENDPOINT FOR EXTRA CREDIT
@main.route('/api/fruits/expired', methods=['GET'])
def get_expired_fruits():
    expired_fruits = Fruit.search(expired_only=True)
    if not expired_fruits:
        return jsonify({'message': 'No expired fruits found'}), 200
    return jsonify([fruit.serialize() for fruit in expired_fruits]), 200